...
def caretaker(request):
    ...
    users = list(users_collection.find({}, {'username': 1, 'email': 1, '_id': 1}))
    ...
    # Handle booking if the request method is POST
    if request.method == 'POST':
        user_email = request.POST.get('user_email')  # Assume this is provided in the form
        caretaker_email = request.POST.get('caretaker_email')  # Caretaker's email to book

        # Update the caretaker collection with user and caretaker email
        caretakers_collection.update_one(
            {'email': caretaker_email},  # Match the caretaker by their email
            {'$set': {'user_email': user_email}}  # Update the user's email
        )

        # Optionally, redirect or provide feedback after booking
        return HttpResponse("Booking successful")

    return render(request, 'list_caretakers.html', {'users': formatted_users})
